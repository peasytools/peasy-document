"""peasy-document API client — Access Peasy Document tools via REST API.

Usage::

    from peasy_document.api import PeasyDocumentAPI

    api = PeasyDocumentAPI()
    tools = api.list_tools()          # paginated: {"count", "next", "results": [...]}
    tool = api.get_tool("doc-convert")
"""
...
class PeasyDocumentAPI:
    """REST API client for peasydocument.com.

    All list methods return DRF-paginated responses::

        {"count": 22, "next": "...?page=2", "previous": null, "results": [...]}

    Use ``page`` and ``limit`` to paginate through results.
    """

    def __init__(self, base_url: str = "https://peasydocument.com") -> None:
